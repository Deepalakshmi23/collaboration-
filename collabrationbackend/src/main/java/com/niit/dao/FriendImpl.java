...
private SessionFactory sessionFactory;
	...
	public List<User> getALLSuggestedUsers(String username) {
	    ...
	    /*String outerQuery="select * from user_batch where username in ";
	   String openParanthesis="("; 
	    	String innerQuery1="select username from user_batch where username!=?";
	    String minusOperator="minus";
	    String innerQuery2="select fromId from friend_batch where toId=? ";
	    String innerQuery3="select toId from friend_batch where fromId=?";
	    
	    */
	    SQLQuery query=session.createSQLQuery("select * from user_batch where username in (select username from user_batch where username!=? minus (select fromId from friend_batch where toId=? union select toId from friend_batch where fromId=?))");
